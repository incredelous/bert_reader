...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    all_results: list. 以namedtuple类型类名为RawResult为元素的一个列表
    n_best_size: int. 记录最好的n_best_size个结果在nbest_predictions.json 文件中
    max_answer_length: int. answer的最大长度
    do_lower_case: bool. 是否将输出的结果小写化
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        train_writer = FeatureWriter(
            ...
            is_training=True)
        # convert_examples_to_features 把文本数据转化为特征[input_ids, input_mask, segment_ids,
        #                                                 start_position, end_position, is_impossible]
        # train_writer.process_feature 把特征以tf.train.Example的形式写入到train.tf_record文件中
